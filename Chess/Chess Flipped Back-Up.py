...
class queenw(object):
    ...
    def possible_moves(self,coords,coords2,colour,piece_backup,Checkmate_pass):
        ...
        if xposition!=9:
            ...
            self.bwmx,self.bwmy=480-self.bwmx,self.bwmy
            ...
            else: self.bwmx,self.bwmy=bo1mx,bo1my
...
class knightw(queenw):
    def possible_moves(self,coords,coords2,colour,piece_backup,Checkmate_pass):
        ...
        if xposition!=9:
            ...
            self.bwmx,self.bwmy=480-self.bwmx,self.bwmy
            if self.Checkmate_pass==True or self.bwmx>list_searchx(Board,self.number)*60 and self.bwmx<list_searchx(Board,self.number)*60+60 and self.bwmy>list_searchy(Board,self.number)*60 and self.bwmy<list_searchy(Board,self.number)*60+60:
                if self.number[3]=="b": piece_backup="knight"+self.number[2]+"b"   
                else: piece_backup="knight"+self.number[2]
                for i in range(-2,3,2):
                    for a in range(-1,3,2):
                        if i!=0:
                            self.coords=knight_moves(xposition,yposition,self.coords,Board,self.colour,i,a)
                            self.coords=knight_moves(xposition,yposition,self.coords,Board,self.colour,a,i)
                    self.coords2.append([(xposition)*60+30,(yposition)*60+30])
                self.coords=Protectors(black_check,white_check,attack,self.coords,attack_positions,Board,two_protectors,self.colour,black_protector,self.number,white_protector)
            else: self.bwmx,self.bwmy=bo1mx,bo1my
        return piece_backup,self.bwmx,self.bwmy
class kingw(bishopw):
    def possible_moves(self,coords,coords2,colour,moves2kb,moves2r2b,moves2r1b):
        self.colour=colour
        self.moves2kb=moves2kb
        self.moves2r2b=moves2r2b
        self.moves2r1b=moves2r1b
        yposition,xposition=list_searchy(Board,self.number),list_searchx(Board,self.number)
        if self.number=="k1b":
                for i in range(-1,2,1):
                    for a in range(-1,2,1):
                        if a!=0 or i!=0: self.coords=king_moves(xposition,yposition,self.coords,Board,self.colour,i,a,all_white)
                self.coords2.append([xposition*60+30,yposition*60+30])
                if self.moves2kb==0 and self.moves2r2b==0:
                    castle=True
                    for i in range (0,len(all_white)):
                ...
                if self.moves2kb==0 and self.moves2r1b==0:
                    castle=True
                    for i in range (0,len(all_white)):
        ...
        if self.number=="k1w":
                for i in range(-1,2,1):
                    for a in range(-1,2,1):
                        if a!=0 or i!=0: self.coords=king_moves(xposition,yposition,self.coords,Board,self.colour,i,a,all_black)
                self.coords2.append([xposition*60+30,yposition*60+30])
                if self.moves2kb==0 and self.moves2r2b==0:
                    castle=True
    ...
    def position(self,coords,coords2,bwmx,bwmy,number):
        self.coords,self.coords2,self.bwmx,self.bwmy,self.number,tg=coords,coords2,bwmx,bwmy,number,0
        if event.type==pygame.KEYDOWN and event.key==pygame.K_SPACE:
                roomx,roomy=pygame.mouse.get_pos()
                roomx,roomy=480-roomx,roomy
                if (roomx-(roomx%60))+30==self.coords2[0][0] and roomy==(roomy-(roomy%60))+30==self.coords2[0][1]:
...
class pawnw(bishopw):
    ...
    def position(self,coords,coords2,bwmx,bwmy,number,pawn_2_spaces_move):
        self.coords,self.coords2,self.bwmx,self.bwmy,self.number,self.pawn_2_spaces_move,tg=coords,coords2,bwmx,bwmy,number,pawn_2_spaces_move,0
        global queenb_moves
        global queenw_moves
        if event.type==pygame.KEYDOWN and event.key==pygame.K_SPACE:
                fv=0
...
class queen1(object):
    ...
    def check(self,q11,q12,q13,q14,q15,q16,q17,q18,black_protector,vc,king,colour,colour2):
        ...
        self.king,self.colour,self.colour2,self.black_protector=king,colour,colour2,black_protector
        yposition,xposition=list_searchy(Board,self.vc),list_searchx(Board,self.vc)
...
class knight1(object):
    ...
    def check(self,kn1,black_protector,vc,king,colour,colour2):
        self.kn1,self.vc,self.colour,self.colour2=kn1,vc,colour,colour2
        yposition,xposition=list_searchy(Board,self.vc),list_searchx(Board,self.vc)
        if xposition!=9:
            for i in range(-2,3,2):
                for a in range(-1,3,2):
                    if i!=0:
                        self.kn1=knight1_attack(yposition,xposition,self.kn1,Board,self.colour,self.colour2,i,a)
                        self.kn1=knight1_attack(yposition,xposition,self.kn1,Board,self.colour,self.colour2,a,i)
class rook1(queen1):
...
class king1(object):
    def __init__(self,k1w,vc,colour,colour2):
        self.k1w,self.vc,self.colour,self.colour2=k1w,vc,colour,colour2
    def check(self,k1w,vc,colour,colour2):
        ...
        if xposition!=9:
            for i in range(-1,2,1):
                for a in range(-1,2,1):
                    if a!=0 or i!=0: self.k1w=knight1_attack(yposition,xposition,self.k1w,Board,self.colour,self.colour2,i,a)
class pawn1w(object):
    def __init__(self,p1,vc,colour,colour2):
        self.p1,self.vc,self.colour,self.colour2=p1,vc,colour,colour2
...
class pawn1b(object):
    def __init__(self,p1b,vc,colour,colour2):
        self.p1b,self.vc,self.colour,self.colour2=p1b,vc,colour,colour2
    def check(self,p1b,vc,colour,colour2):
        self.p1b,self.vc,self.colour,self.colour2=p1b,vc,colour,colour2
        yposition,xposition=list_searchy(Board,self.vc),list_searchx(Board,self.vc)
        if yposition!=9 or xposition!=9:
            if (yposition-1)<8 and (xposition+1)<8 and (yposition-1)>-1 and (xposition+1)>-1 and Board[(yposition-1)][(xposition+1)][1]=="" or (yposition-1)<8 and (xposition+1)<8 and (yposition-1)>-1 and (xposition+1)>-1 and Board[(yposition-1)][(xposition+1)][1]==self.colour or (yposition-1)<8 and (xposition+1)<8 and (yposition-1)>-1 and (xposition+1)>-1 and Board[(yposition-1)][(xposition+1)][1]==self.colour2:
...
class attack_class_and_all_white(object):
    def __init__(self,vc,attack_piece,black_check,attack,attack_positions,all_white):
        self.vc,self.attack_piece,self.black_check,self.attack,self.attack_positions,self.all_white=vc,attack_piece,black_check,attack,attack_positions,all_white
    def attack_class1(self,vc,attack_piece,black_check,attack,attack_positions,all_white):
        if self.vc[len(vc)-1]=="b":
...
def board(r1wmx,r1wmy,k1wmx,k1wmy,r2wmx,r2wmy,r2bmx,r2bmy,r1bmx,r1bmy,b1wmx,b1wmy,b2wmx,b2wmy,b1bmx,b1bmy,b2bmx,b2bmy,kn1wmx,kn1wmy,kn2wmx,kn2wmy,kn1bmx,kn1bmy,kn2bmx,kn2bmy,q1wmx,q1wmy,q1bmx,q1bmy,k1bmx,k1bmy,p8wmx,p8wmy,p7wmx,p7wmy,p6wmx,p6wmy,p5wmx,p5wmy,p4wmx,p4wmy,p3wmx,p3wmy,p2wmx,p2wmy,p1wmx,p1wmy,p5bmx,p5bmy,p1bmx,p1bmy,p2bmx,p2bmy,p3bmx,p3bmy,p4bmx,p4bmy,p6bmx,p6bmy,p7bmx,p7bmy,p8bmx,p8bmy,q2wmx,q2wmy,q2bmx,q2bmy,r3wmx,r3wmy,r4wmx,r4wmy,r5wmx,r5wmy,b3wmx,b3wmy,b4wmx,b4wmy,b5wmx,b5wmy,kn3wmx,kn3wmy,kn4wmx,kn4wmy,kn5wmx,kn5wmy,q3wmx,q3wmy,q4wmx,q4wmy,r3bmx,r3bmy,r4bmx,r4bmy,r5bmx,r5bmy,b3bmx,b3bmy,b4bmx,b4bmy,b5bmx,b5bmy,kn3bmx,kn3bmy,kn4bmx,kn4bmy,kn5bmx,kn5bmy,q3bmx,q3bmy,q4bmx,q4bmy):
    ...
    win.blit(bg,(0,0))
    position_display(k1bmx,k1bmy,"k1b",king_black),position_display(k1wmx,k1wmy,"k1w",king_white)
    position_display(q1bmx,q1bmy,"q1b",queen_black),position_display(q2bmx,q2bmy,"q2b",queen_black2)
    position_display(q3bmx,q3bmy,"q3b",queen_black3),position_display(q4bmx,q4bmy,"q4b",queen_black4)
    position_display(r1bmx,r1bmy,"r1b",rook_black1),position_display(r2bmx,r2bmy,"r2b",rook_black2)
    position_display(b1bmx,b1bmy,"b1b",bishop_black1),position_display(b2bmx,b2bmy,"b2b",bishop_black2)
    position_display(kn1bmx,kn1bmy,"kn1b",knight_black1),position_display(kn2bmx,kn2bmy,"kn2b",knight_black2)
    position_display(p1bmx,p1bmy,"p1b",pawn_black1),position_display(p2bmx,p2bmy,"p2b",pawn_black2)
    position_display(p3bmx,p3bmy,"p3b",pawn_black3),position_display(p4bmx,p4bmy,"p4b",pawn_black4)
    position_display(p5bmx,p5bmy,"p5b",pawn_black5),position_display(p6bmx,p6bmy,"p6b",pawn_black6)
    position_display(p7bmx,p7bmy,"p7b",pawn_black7),position_display(p8bmx,p8bmy,"p8b",pawn_black8)
    position_display(q1wmx,q1wmy,"q1w",queen_white),position_display(q2wmx,q2wmy,"q2w",queen_white2)
    position_display(q3wmx,q3wmy,"q3w",queen_white3),position_display(q4wmx,q4wmy,"q4w",queen_white4)
    position_display(r1wmx,r1wmy,"r1w",rook_white1),position_display(r2wmx,r2wmy,"r2w",rook_white2)
    position_display(b1wmx,b1wmy,"b1w",bishop_white1),position_display(b2wmx,b2wmy,"b2w",bishop_white2)
    position_display(kn1wmx,kn1wmy,"kn1w",knight_white1),position_display(kn2wmx,kn2wmy,"kn2w",knight_white2)
    position_display(p1wmx,p1wmy,"p1w",pawn_white1),position_display(p2wmx,p2wmy,"p2w",pawn_white2)
    position_display(p3wmx,p3wmy,"p3w",pawn_white3),position_display(p4wmx,p4wmy,"p4w",pawn_white4)
    position_display(p5wmx,p5wmy,"p5w",pawn_white5),position_display(p6wmx,p6wmy,"p6w",pawn_white6)
    position_display(p7wmx,p7wmy,"p7w",pawn_white7),position_display(p8wmx,p8wmy,"p8w",pawn_white8)
    win.blit(pygame.transform.flip(win, True, False),(0,0))
...
def attack_append(piece,attack,Board,piece_attack):
    if piece[len(piece)-1]=="w": yposition,xposition=(list_searchy(Board,"k1b"))*60,(list_searchx(Board,"k1b")*60)
    else: yposition,xposition=(list_searchy(Board,"k1w"))*60,(list_searchx(Board,"k1w")*60)
    for ju in range (0,len(piece_attack)):
        if xposition==piece_attack[ju][0] and yposition==piece_attack[ju][1]: attack.append([piece])
    return attack
def all_white_append(piece_attack,all_white):
    for i in range (0,len(piece_attack)):
        if piece_attack!=[]: all_white.append(piece_attack[i])
    return all_white
def piece_place(piece,b1wmx,b1wmy,hb,white_check,moves,rfv,coords,coords2,vc,piece2,class_piece,moves2r1w):
    if piece==piece2:
        b1wmx,b1wmy=pygame.mouse.get_pos()
        b1wmx,b1wmy=480-b1wmx,b1wmy
        tg=class_piece.position(coords,coords2,convertx(b1wmx),converty(b1wmy),vc)                    
        if tg==2:
            if piece=="rook1" or piece=="rook2" and piece=="rook1b" or piece=="rook2b": moves2r1w=moves2r1w+1
    ...
    return piece,b1wmx,b1wmy,hb,white_check,moves,rfv,coords,coords2,vc,class_piece,moves2r1w
def piece_place_pawn(piece,b1wmx,b1wmy,hb,white_check,moves,rfv,coords,coords2,vc,piece2,class_piece,pawn_2_spaces_move,moves21):
    if piece==piece2:
        ...
        if tg==2: hb,white_check,moves,moves21=0,False,moves+1,moves21+1
        if tg==3 or tg==2 or tg==1: coords,coords2,piece,rfv,b1wmx,b1wmy=[],[],"",1,(list_searchx(Board,vc))*60,(list_searchy(Board,vc))*60
...
knight_white1,knight_white2,knight_white3,knight_white4,knight_white5=pygame.image.load("knight_white4.png"),pygame.image.load("knight_white4.png"),pygame.image.load("knight_white4.png"),pygame.image.load("knight_white4.png"),pygame.image.load("knight_white4.png")
knight_black1,knight_black2,knight_black3,knight_black4,knight_black5=pygame.image.load("knight_black4.png"),pygame.image.load("knight_black4.png"),pygame.image.load("knight_black4.png"),pygame.image.load("knight_black4.png"),pygame.image.load("knight_black4.png")
...
moves,hb,hw,black,run,piece=0,0,0,(255,0,0),True,""
...
pawn_2_spaces_move,pos_spaces,two_protectors=[],[],False
jkl=0
attack_positions=[]
...
while run:
    for event in pygame.event.get():
        ...
        if hb==0:
            ...
            black_protector,pos_spaces,all_white=[],[],[]
        ...
        if hw==0:
            ...
            hw=1
        white_stalemate,white_checkmate=False,False
        coords_backup,coords_backup2=coords,coords2
        if moves==0:
            coords,coords2=[],[]
            ...
            King1w.possible_moves(coords,coords2,"b",moves2kw,moves2r2w,moves2r1w)
        ...
        if moves==1:
            ...
            King1b.possible_moves(coords,coords2,"w",moves2kb,moves2r2b,moves2r1b)
        ...
        if moves==0:
            ...
            piece,b1wmx,b1wmy,hb,white_check,moves,rfv,coords,coords2,vc,Bishop1w,moves2r1w=piece_place(piece,b1wmx,b1wmy,hb,white_check,moves,rfv,coords,coords2,"b1w","bishop1",Bishop1w,moves2r1w)
            piece,b2wmx,b2wmy,hb,white_check,moves,rfv,coords,coords2,vc,Bishop2w,moves2r1w=piece_place(piece,b2wmx,b2wmy,hb,white_check,moves,rfv,coords,coords2,"b2w","bishop2",Bishop2w,moves2r1w)
            piece,kn1wmx,kn1wmy,hb,white_check,moves,rfv,coords,coords2,vc,Knight1w,moves2r1w=piece_place(piece,kn1wmx,kn1wmy,hb,white_check,moves,rfv,coords,coords2,"kn1w","knight1",Knight1w,moves2r1w)
            piece,kn2wmx,kn2wmy,hb,white_check,moves,rfv,coords,coords2,vc,Knight2w,moves2r1w=piece_place(piece,kn2wmx,kn2wmy,hb,white_check,moves,rfv,coords,coords2,"kn2w","knight2",Knight2w,moves2r1w)
            piece,r1wmx,r1wmy,hb,white_check,moves,rfv,coords,coords2,vc,Rook1w,moves2r1w=piece_place(piece,r1wmx,r1wmy,hb,white_check,moves,rfv,coords,coords2,"r1w","rook1",Rook1w,moves2r1w)
            piece,r2wmx,r2wmy,hb,white_check,moves,rfv,coords,coords2,vc,Rook2w,moves2r2w=piece_place(piece,r2wmx,r2wmy,hb,white_check,moves,rfv,coords,coords2,"r2w","rook2",Rook2w,moves2r2w)
            piece,q1wmx,q1wmy,hb,white_check,moves,rfv,coords,coords2,vc,Queen1w,moves2r1w=piece_place(piece,q1wmx,q1wmy,hb,white_check,moves,rfv,coords,coords2,"q1w","queen1",Queen1w,moves2r1w)
            piece,q2wmx,q2wmy,hb,white_check,moves,rfv,coords,coords2,vc,Queen2w,moves2r1w=piece_place(piece,q2wmx,q2wmy,hb,white_check,moves,rfv,coords,coords2,"q2w","queen2",Queen2w,moves2r1w)
            piece,q3wmx,q3wmy,hb,white_check,moves,rfv,coords,coords2,vc,Queen3w,moves2r1w=piece_place(piece,q3wmx,q3wmy,hb,white_check,moves,rfv,coords,coords2,"q3w","queen3",Queen3w,moves2r1w)
            piece,q4wmx,q4wmy,hb,white_check,moves,rfv,coords,coords2,vc,Queen4w,moves2r1w=piece_place(piece,q4wmx,q4wmy,hb,white_check,moves,rfv,coords,coords2,"q4w","queen4",Queen4w,moves2r1w)
            ...
            if piece=="king1":
                ...
                if tg==2:
                    ...
                    hb,white_check,moves,moves2kw=0,False,moves+1,moves2kw+1
                if tg==3 or tg==2 or tg==1:
        ...
        if moves==1:
            ...
            if piece!="":          
                piece,b1bmx,b1bmy,hw,black_check,moves,rfv,coords,coords2,vc,Bishop1b,moves2r1w=piece_place(piece,b1bmx,b1bmy,hw,black_check,moves,rfv,coords,coords2,"b1b","bishop1b",Bishop1b,moves2r1w)
                piece,b2bmx,b2bmy,hw,black_check,moves,rfv,coords,coords2,vc,Bishop2b,moves2r1w=piece_place(piece,b2bmx,b2bmy,hw,black_check,moves,rfv,coords,coords2,"b2b","bishop2b",Bishop2b,moves2r1w)
                piece,kn1bmx,kn1bmy,hw,black_check,moves,rfv,coords,coords2,vc,Knight1b,moves2r1w=piece_place(piece,kn1bmx,kn1bmy,hw,black_check,moves,rfv,coords,coords2,"kn1b","knight1b",Knight1b,moves2r1w)
                piece,kn2bmx,kn2bmy,hw,black_check,moves,rfv,coords,coords2,vc,Knight2b,moves2r1w=piece_place(piece,kn2bmx,kn2bmy,hw,black_check,moves,rfv,coords,coords2,"kn2b","knight2b",Knight2b,moves2r1w)
                piece,r1bmx,r1bmy,hw,black_check,moves,rfv,coords,coords2,vc,Rook1b,moves2r1b=piece_place(piece,r1bmx,r1bmy,hw,black_check,moves,rfv,coords,coords2,"r1b","rook1b",Rook1b,moves2r1b)
                piece,r2bmx,r2bmy,hw,black_check,moves,rfv,coords,coords2,vc,Rook1b,moves2r2b=piece_place(piece,r2bmx,r2bmy,hw,black_check,moves,rfv,coords,coords2,"r2b","rook2b",Rook2b,moves2r2b)
                piece,q1bmx,q1bmy,hw,black_check,moves,rfv,coords,coords2,vc,Queen1b,moves2r1w=piece_place(piece,q1bmx,q1bmy,hw,black_check,moves,rfv,coords,coords2,"q1b","queen1b",Queen1b,moves2r1w)
                piece,q2bmx,q2bmy,hw,black_check,moves,rfv,coords,coords2,vc,Queen2b,moves2r1w=piece_place(piece,q2bmx,q2bmy,hw,black_check,moves,rfv,coords,coords2,"q2b","queen2b",Queen2b,moves2r1w)
                piece,q3bmx,q3bmy,hw,black_check,moves,rfv,coords,coords2,vc,Queen3b,moves2r1w=piece_place(piece,q3bmx,q3bmy,hw,black_check,moves,rfv,coords,coords2,"q3b","queen3b",Queen3b,moves2r1w)
                piece,q4bmx,q4bmy,hw,black_check,moves,rfv,coords,coords2,vc,Queen4b,moves2r1w=piece_place(piece,q4bmx,q4bmy,hw,black_check,moves,rfv,coords,coords2,"q4b","queen4b",Queen4b,moves2r1w)
            ...
            if piece=="king1b":
                ...
                if tg==2:
                    ...
                    hw,black_check,moves,moves2kb=0,False,moves+1,moves2kb+1
                if tg==3 or tg==2 or tg==1:
